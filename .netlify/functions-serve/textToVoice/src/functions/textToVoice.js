...
// functions/textToVoice.js
var textToVoice_exports = {};
__export(textToVoice_exports, {
  convertTextToSpeech: () => convertTextToSpeech
});
module.exports = __toCommonJS(textToVoice_exports);
...
function readableStreamAsyncIterable(stream4) {
...
var getDefaultAgent = (url2) => {
  if (defaultHttpsAgent && url2.startsWith("https"))
...
var APIClient = class {
  ...
  buildRequest(options) {
    ...
    const url2 = this.buildURL(path, query);
    if ("timeout" in options)
      validatePositiveInteger("timeout", options.timeout);
    const timeout = (_a2 = options.timeout) !== null && _a2 !== void 0 ? _a2 : this.timeout;
    const httpAgent = (_c = (_b = options.httpAgent) !== null && _b !== void 0 ? _b : this.httpAgent) !== null && _c !== void 0 ? _c : getDefaultAgent(url2);
   ...
   */
  async prepareRequest(request, { url: url2 }) {
  }
  ...
  async makeRequest(optionsInput, retriesRemaining) {
    ...
    const { req, url: url2, timeout } = this.buildRequest(options);
    await this.prepareRequest(req, { url: url2 });
    ...
    if (!response.ok) {
      ...
      debug("response", response.status, url2, responseHeaders, errMessage);
  ...
  buildURL(path, query) {
    const url2 = isAbsoluteURL(path) ? new URL(path) : new URL(this.baseURL + (this.baseURL.endsWith("/") && path.startsWith("/") ? path.slice(1) : path));
  ...
  async fetchWithTimeout(url2, init, ms, controller) {
    ...
    return this.getRequestClient().fetch(url2, { signal: controller.signal, ...options }).finally(() => {
...
var FormDataPart = class {
  constructor(name, value) {
    const { escapeName: escapeName2 } = this.constructor;
    const isStringValue = utils_default.isString(value);
    let headers = `Content-Disposition: form-data; name="${escapeName2(name)}"${!isStringValue && value.name ? `; filename="${escapeName2(value.name)}"` : ""}${CRLF}`;
    if (isStringValue) {
...
async function convertTextToSpeech(inputText) {
  try {
    ...
    const response = await axios_default.post(apiUrl, requestBody, {
      ...
      // Tell Axios to expect binary data
    });
...
formdata-node/lib/esm/Blob.js:
  (*! Based on fetch-blob. MIT License. Jimmy Wärting <https://jimmy.warting.se/opensource> & David Frank *)
*/
//# sourceMappingURL=textToVoice.js.map
